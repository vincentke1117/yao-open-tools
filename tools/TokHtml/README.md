...
7. 编辑后 600ms 防抖自动保存。保存前会生成版本快照。
8. 页面列表默认每页 20 条，底部翻页条支持上一页、下一页和页码跳转。
9. 访问 `/<slug>` 或编辑地址时，会自动统计访问次数，并显示在列表“目录名称”后。
10. 点击删除会把页面移入“回收站”，对应生成文件会移动到 `data/trash/` 下，原本的 `/<slug>` 不再可访问；在回收站里点击“恢复”可重新展示。
11. 在“设置”的“线上绑定”里填写同类线上程序的 API 地址和 Token 后，页面列表可一键上传当前 HTML 到线上程序。
12. API 支持版本列表和恢复：
